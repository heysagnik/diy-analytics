...
import Alert, { ALERT_METRICS, ALERT_THRESHOLD_TYPES } from "../../../../../models/Alert";
...
export async function GET(
  ...
  try {
    ...
    const alerts = await Alert.find({ projectId: id }).sort({ createdAt: -1 }).lean();
...
export async function POST(
  ...
  try {
    ...
    if (!ALERT_THRESHOLD_TYPES.includes(thresholdType)) {
      return NextResponse.json({ error: `Threshold type must be one of: ${ALERT_THRESHOLD_TYPES.join(', ')}` }, { status: 400 });
    }
    if (typeof thresholdValue !== "number" || !isFinite(thresholdValue) || thresholdValue < 0) {
