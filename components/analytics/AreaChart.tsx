...
import { type ChartGranularity, formatAxisLabel, formatTooltipLabel } from './chartLabels';
...
export default function AreaChart({
  ...
  const chartSummary = useMemo(() => {
    const seriesSummary = seriesData
      .map((series) => {
  ...
  if (!seriesData.length || !chartData.length) {
    return (
      <div
        className="relative w-full h-full flex items-center justify-center text-muted-foreground"
  ...
  return (
    <section className={`w-full h-full ${className}`} style={{ height: `${height}px` }} aria-label="Traffic area chart">
          ...
          <RechartsAreaChart data={chartData} margin={chartMargin}>
            <defs>
              {seriesData.map((series, index) => (
                <linearGradient
                  key={series.name}
                  id={`${gradientIdPrefix}-gradient-${index}`}
                  x1="0"
                  y1="0"
                  x2="0"
                  y2="1"
