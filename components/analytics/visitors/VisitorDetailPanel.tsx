...
export const VisitorDetailPanel: React.FC<VisitorDetailPanelProps> = ({ projectId, userId, onClose, embedded = false }) => {
  ...
  const { days: heatmapDays, loading: loadingHeatmap, error: heatmapError } = useVisitorHeatmap(projectId, userId, heatmapYear);
  ...
  const header = (
    <div className="flex items-center justify-between gap-3 border-b border-border bg-surface-secondary/50 px-5 py-3.5">
      <div className="min-w-0 flex items-center gap-3">
        ...
        <span className="font-mono text-xs font-semibold text-foreground truncate tracking-tight" title={userId ?? undefined}>
          {userId}
        </span>
      </div>
      {embedded && (
          ...
          <div className="px-5 pb-5 flex flex-col gap-6">
            <div className="grid grid-cols-2 gap-3">
              <div className="surface-tile p-3.5">
                ...
                <p className="text-xl font-semibold text-foreground tabular-nums font-display">{detail.totalSessions}</p>
              </div>
              <div className="surface-tile p-3.5">
                <p className="label-eyebrow text-muted-foreground mb-1">Pageviews</p>
                <p className="text-xl font-semibold text-foreground tabular-nums font-display">{detail.totalPageviews}</p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="surface-tile divide-y divide-border overflow-hidden">
                ...
                {detail.devices.length === 0 && detail.browsers.length === 0 && detail.operatingSystems.length === 0 && (
            ...
            <div>
              ...
              {loadingHeatmap ? (
                ...
                <p className="rounded-lg border border-danger/20 bg-danger/5 p-3 text-xs font-medium text-danger" role="status">
            ...
            <div>
              <div className="flex items-center justify-between gap-2 mb-2">
                ...
                <span className="text-xs font-medium text-muted-foreground tabular-nums">{detail.sessions.length} sessions</span>
              </div>
                ...
                {detail.sessions.map((s: VisitorSession, idx: number) => (
                  <div key={s.sessionId || idx} className="p-2.5 text-xs hover:bg-surface-secondary/40 transition-colors duration-150 ease-out flex flex-col gap-1">
                    ...
                    {s.paths && s.paths.length > 0 && (
                      <p className="font-mono text-xs text-muted-foreground/80 truncate pt-0.5" title={s.paths.join(', ')}>
  ...
  if (!isDesktop) {
    return (
      <Drawer open={Boolean(userId)} onOpenChange={(open) => { if (!open) onClose(); }}>
